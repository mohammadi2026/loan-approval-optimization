...
actual_model = model_bundle['logistic_model']
# We use the threshold from the bundle, or default to 0.7 if not found
CUSTOM_THRESHOLD = model_bundle.get('threshold', 0.7)
...
# Categorical inputs 
reason = st.selectbox("Reason for Loan", ["debt_consolidation", "credit_card_refinancing", "home_improvement", "major_purchase", "other"])
...
# --- FIX 2: USE ACTUAL_MODEL FOR FEATURE NAMES ---
model_columns = actual_model.feature_names_in_
for col in model_columns:
    if col not in input_data_encoded.columns:
        input_data_encoded[col] = 0

# 3. Reorder/filter columns
input_data_encoded = input_data_encoded[model_columns]

# 4. Predict button
if st.button("Evaluate Loan"):
    # --- FIX 3: CALL PREDICT_PROBA ON THE ACTUAL MODEL ---
    proba = actual_model.predict_proba(input_data_encoded)[0][1]
    ...
    """
LOGIC CONTEXT: Handling Unknown Inputs
Example: User selects Employment_Sector = 'Agriculture', but the model was 
only trained on 'Financials', 'Energy', and 'IT'.

1. pd.get_dummies creates: 'Employment_Sector_Agriculture' = 1.
2. The loop below adds 'Employment_Sector_Financials' = 0, 'Employment_Sector_Energy' = 0, etc.
3. The final filtering step DROPS 'Employment_Sector_Agriculture' because it is 
   not in actual_model.feature_names_in_.

Result: The model receives 0s for all known sectors, effectively treating 
'Agriculture' as the "Other/Baseline" category without crashing the app.
"""
